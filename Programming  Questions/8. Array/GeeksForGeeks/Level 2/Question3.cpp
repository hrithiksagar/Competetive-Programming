...
int pairsumcount(int arr[], int n, int sum)
{
    unordered_map<int, int> m; // m is the name of the map
    // Storing all the content of all the elements in map m
    for (int i = 0; i < n; i++)
    {
        m[arr[i]]++;
    }
    int twice_count = 0;
    // iterating through each element and incrementing count (Every pair is counted twice)
    for (int i = 0; i < n; i++)
    {
        twice_count += m[sum - arr[i]];
        // if (arr[i], arr[i]) pair satisfies the condition, then we need to ensure taht count is decreased by one such that the (arr[i], arr[j]) pair is not considered
        if(sum - arr[i] == arr[i])
        {
            twice_count--;
        }
